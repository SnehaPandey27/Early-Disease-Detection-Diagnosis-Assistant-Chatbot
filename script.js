...
async function submitFollowupAnswers() {
    ...
    try {
        ...
        if (data.final_response) {
            formatResponse("CureBot", data.final_response, "bot-message");
        }
    } catch (error) {
        console.error("Error:", error);
    }
...
// Function to format the bot's response with typing animation
function formatResponse(sender, message, className) {
    ...
    messageContent.innerHTML = ""; // Start empty for animation

    messageContainer.appendChild(senderElement);
    messageContainer.appendChild(messageContent);
    chatBox.appendChild(messageContainer);

    // Format the message while keeping line breaks
    let formattedText = message
        .replace(/\n/g, " <br> ") // Add space to prevent splitting issues
        .replace(/(\d+\.)/g, " <br><strong>$1</strong> "); // Highlight numbered points

    let words = formattedText.split(" "); // Split message into words
    let index = 0;
    ...
    function typeNextWord() {
        if (index < words.length) {
            ...
            index++;
            setTimeout(typeNextWord, 50); // Adjust speed here (lower = faster)
        }
    }
